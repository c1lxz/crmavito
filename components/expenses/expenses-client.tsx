...
import { Plus, Search, ShoppingCart, Truck, Megaphone, Package, Percent, DollarSign, MoreHorizontal } from "lucide-react";
  ...
  return (
    <div className="bg-background min-h-screen">
        ...
        <div className="px-4 pb-3 space-y-2">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
            <button
              ...
              className={`px-3 py-1 rounded-full text-xs whitespace-nowrap font-medium ${catFilter === "ALL" ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground"}`}
            >
              Все
            </button>
            {CATEGORIES.map(([v, l]) => (
              <button
                ...
                className={`px-3 py-1 rounded-full text-xs whitespace-nowrap font-medium ${catFilter === v ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground"}`}
              >
                {l}
              </button>
            ))}
      ...
      <div className="px-4 py-3">
        <h2 className="text-sm font-semibold text-muted-foreground mb-2">Расходы за месяц</h2>
        <div className="grid grid-cols-4 gap-2 mb-2">
          <Card className="col-span-4 bg-primary/5 border-primary/20">
            <CardContent className="p-3 text-center">
              <p className="text-xs text-muted-foreground">Всего</p>
              <p className="text-lg font-bold text-primary">{formatRub(initialData.monthTotal)}</p>
            </CardContent>
          </Card>
        </div>
        <div className="grid grid-cols-3 gap-2">
          {CATEGORIES.map(([cat, label]) => (
            <Card key={cat} className="border-0 bg-muted/50">
