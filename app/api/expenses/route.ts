...
export async function GET(req: NextRequest) {
  ...
  if (dateFrom || dateTo) {
    where.date = {};
    if (dateFrom) (where.date as Record<string, Date>).gte = parseDatabaseDateInput(dateFrom);
    if (dateTo) (where.date as Record<string, Date>).lte = parseDatabaseDateInput(dateTo, true);
  }
