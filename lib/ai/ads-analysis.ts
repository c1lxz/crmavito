...
export function buildAdsAnalysisPrompt(input: AdsAnalysisInput): string {
  ...
  return [
    ...
    "Сформируй только 4–6 ключевых метрик аккаунта, 3–5 сильных портфельных выводов и 6–10 приоритетных действий. Не повторяй одну мысль в разных разделах. Действия должны покрывать разные рычаги: контент, видео, цена, продвижение, дубли/ассортимент; не заполняй отчёт одними заменами описания.",
