...
function buildMetaPromptRequest(
  ...
  return [
    ...
    labelStyleReference?.trim()
      ? `Neck-label aesthetic reference: ${labelStyleReference.trim()}. Add exactly one small, single-line heat-transfer marking inside the back neck reading exactly "CUSTOM MADE". It must be ink printed directly on the fabric, never a sewn or woven tag. This is a hard text constraint: the entire image may contain only those exact two words. Require clean blank fabric immediately above, below and around the marking. Explicitly prohibit a second line, microtext, subtitle, decorative letters, pseudo-words, care text, duplicate marking or any extra branding. Use only broad high-level aesthetic cues from the reference. Invent original typography and spacing; never render the reference name, its logo, monogram or distinctive trade dress.`
