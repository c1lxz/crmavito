...
function buildMetaPromptRequest(
  ...
  return [
    ...
    labelStyleReference?.trim()
      ? `Internal heat-transfer identification hint: ${labelStyleReference.trim()}. There are no hang tags, paper tags, sewn labels or woven tabs. The only label is printed inside the back-neck panel and must remain physically hidden in exterior product photos.`
