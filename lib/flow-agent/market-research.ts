...
export function buildOriginalDesignPrompt(
  ...
  return [
    ...
    "ADULT STREETWEAR LOCK: make sharp trap/archive fashion, never children's, fantasy or soft botanical merch. No animals, birds, insects, owls, mascots, fantasy creatures, moon/zodiac/tarot imagery, flowers, branches, cute faces or cartoons. A literal subject on a winner teaches scale and hierarchy only; never repeat it or replace it with another creature or decorative nature illustration.",
...
function fallbackDesignConcept(note?: string) {
  const concepts = [
    ["CLOSED CIRCUIT", "a narrow xerox CCTV crop of an anonymous hooded shoulder and timestamp 03:17, cut by one oxblood scan line", "a larger two-frame surveillance contact sheet with hard black crops, serial digits 0317 and the exact words CLOSED CIRCUIT"],
    ["BACK ROOM", "a flash-burned anonymous side-profile crop interrupted by a black censor strip and tiny exact text BACK ROOM", "a larger off-register nightclub portrait crop with torn flyer edges, one oxblood registration bar and no invented logo"],
    ["ACCESS DENIED", "a compact distressed security-sticker composition with barcode fragments and the exact words ACCESS DENIED", "an asymmetric stack of access stamps, serial numbers and one cropped anonymous hand, printed like a damaged club pass"],
    ["COLD LINK", "a hard-flash macro photograph of one steel chain link with a narrow condensed serial caption", "a larger diagonal chain macro broken by an oxblood photocopy stripe and exact text COLD LINK / 04"],
    ["LAST ENTRY", "a torn black club wristband scan with one stamped number 23 and the exact words LAST ENTRY", "a layered after-hours flyer composition with a cropped doorway silhouette, entry stamp and restrained serial typography"],
    ["SERVICE EXIT", "a grainy CCTV crop of an industrial exit door with one distorted arrow and exact text SERVICE EXIT", "a larger concrete corridor still with hard flash falloff, timestamp digits and a narrow oxblood warning line"],
    ["PRESSURE TEST", "a close xerox crop of a gloved hand gripping a steel cable, with the exact small caption PRESSURE TEST", "two cropped hands pulling the same cable across black negative space, overprinted with calibration ticks and serial type"],
    ["PRIVATE NUMBER", "a compact thermal-receipt fragment with blacked-out data, one visible serial 008 and exact text PRIVATE NUMBER", "a larger torn receipt and security-envelope collage with distorted condensed type and one oxblood cancellation bar"],
    ["BLACK RECEIPT", "a flash-lit anonymous jaw and collar crop crossed by a narrow thermal-receipt strip", "a larger asymmetric portrait-and-receipt xerox composition with exact text BLACK RECEIPT and controlled toner damage"],
    ["REDLINE CONTACT", "one cropped human hand wrapping a thin oxblood cable around two fingers, rendered as a hard-flash xerox photograph", "two anonymous hands connected by the taut cable, interrupted by serial typography and one red registration line"],
  ] as const;
