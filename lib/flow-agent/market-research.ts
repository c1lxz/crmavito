...
export function buildOriginalDesignPrompt(
  ...
  return [
    ...
    "If the views show graphics on both sides, invent a coordinated but original front-and-back print system. Across the four independent runs vary front, back and angled presentation while returning exactly one image per run.",
    ...
    `Keep the garment category and realistic construction believable. Preserve the background identity, perspective and light from REFERENCE IMAGE ${referenceCount + 1}.`,
    ...
    ...(designNote?.trim() ? [`Mandatory creative direction: ${designNote.trim()}`] : []),
    ...
    `Use no words, letters or numbers${labelStyleReference?.trim() ? " except the requested original neck label" : ", neck-label text"} and no fake branding. No mockup labels, watermarks, UI, borders or explanatory text. Return only the final image.`,
  ].join("\n");
}
